    ...
    <section className="container-x py-14">
      <div className="mx-auto max-w-md rounded-2xl border border-ink-100 bg-white p-8 shadow-card">
        ...
        <p className="mt-1 text-sm text-ink-500">
          Rápido, gratuito e você mantém pedidos e endereços salvos.
        </p>

        <div className="mt-6">
          <CustomerSignupForm redirect={safeTo} />
        </div>

        <p className="mt-6 text-center text-sm text-ink-700">
          Já tem conta?{' '}
          <Link
            href={`/conta/entrar${safeTo !== '/conta' ? `?redirect=${encodeURIComponent(safeTo)}` : ''}`}
